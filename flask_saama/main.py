...
def user_workouts():
    ...
    print("bbbbbbbbbbbbb", user)
    work_outs = user.work_outs
    work_outs = Workout.query.filter_by(author=user).paginate(page=page, per_page=3)
    print("ffffffffffff", work_outs)
    return render_template('all_workouts.html', work_outs=work_outs,user=user)
